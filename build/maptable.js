...
this.d3.maptable = (function () {
  ...
  function _ref5() {
    ...
    var hor = d3.extent(this.maptable.data, _ref12);
    var ver = d3.extent(this.maptable.data, _ref13);
  ...
  function _ref7() {
    ...
    if (this.markers) {
      // markers
      d3.selectAll('.mt-map-marker').each(_ref14);
      // countries
      d3.selectAll('.mt-map-country').each(function (d) {
        // stroke
        if (d.prop['stroke-width']) {
          d3.select(_this2).attr('stroke-width', d.prop['stroke-width'] / self.scale_attributes());
        }
      });
    }
  }

  function _ref8(obj, key, datum, data) {
    function _ref16(d) {
    ...
    if (obj.attr[key] instanceof Array) {
      var domain = d3.extent(data, _ref15);
      ...
      var filteredData = data.filter(_ref16);
  ...
  function _ref9() {
    ...
    var dataMarkers = d3.nest().key(this.options.markers.groupBy ? this.options.markers.groupBy : _ref17).entries(this.maptable.data).filter(_ref18);
    ...
    if (this.options.markers.attr) {
      Object.keys(this.options.markers.attr).forEach(function (key) {
        markerUpdate.attr(key, function (datum) {
          if (!datum.prop) datum.prop = {};
          datum.prop[key] = _this3.getScaledValue(_this3.options.markers, key, datum, dataMarkers);
  ...
  function _ref10() {
    ...
    if (this.options.countries.attr) {
      (function () {
        var dataCountries = [];
        var dataCountriesAssoc = {};
        if (_this4.options.countries.groupBy) {
          dataCountries = d3.nest().key(_this4.options.countries.groupBy).entries(_this4.maptable.data);
          for (var i = 0; i < dataCountries.length; i++) {
            dataCountriesAssoc[dataCountries[i].key] = dataCountries[i].values;
          }
        }

        if (_this4.legendObject) {
          var domain = d3.extent(dataCountries, function (d) {
            return _this4.options.countries.rollup(d.values);
          });
          _this4.legendObject.updateExtents(domain);
        }
        ...
        var countryItem = d3.selectAll('.mt-map-country').each(function (datum) {
          ...
          Object.keys(self.options.countries.attr).forEach(function (key) {
            d3.select(_this5).attr(key, function () {
              if (!datum.prop) datum.prop = {};
        ...
        if (_this4.legendObject) {
          countryItem.on('mouseover', function (datum) {
            return _this4.legendObject.indiceChange(datum.value);
          }).on('mouseout', function () {
            return _this4.legendObject.indiceChange(NaN);
          });
        }

        if (_this4.options.countries.tooltip) {
          _this4.activateTooltip(countryItem, _this4.options.countries.tooltip);
        }
      })();
    }
  }
  ...
  function _ref11(target, tooltipContent, cb) {
    var _this6 = this;

    target.on('mousemove', function (d) {
      var mousePosition = d3.mouse(_this6.svg.node()).map(_ref19);
  ...
  var GeoMap = function () {
    function GeoMap(maptable, options, jsonWorld) {
      ...
      this.maptable.rawData.forEach(function (d) {
        d.longitude = parseFloat(d[_this.options.longitudeKey]);
        d.latitude = parseFloat(d[_this.options.latitudeKey]);
        var coord = _this.projection([d.longitude, d.latitude]);
        d.x = coord[0];
        d.y = coord[1];
        return d;
      });
  ...
  function _ref2$3(filterName, replaceNode) {
  ...
  function _ref6$1(filterName) {
    ...
    utils.appendOptions(filterNameSelect, possibleFilters.map(_ref12$1));
    ...
    if (filterOptions.type !== 'field' && filterOptions.type !== 'dropdown') {
      ...
      utils.appendOptions(filterRange, ['any', '=', '≠', '<', '>', '≤', '≥', 'BETWEEN'].map(_ref13$1));
      filterRange.addEventListener('change', _ref14);
  ...
  function _ref10$1() {
    ...
    for (var i = 0; i < filterNameSelects.length; i++) {
      ...
      utils.appendOptions(filterNameSelect, possibleFilters.map(_ref15));
      filterNameSelect.value = filterName;
    }
  ...
  function _ref2$4(d) {
    return !d.wrap ? 'white-space:nowrap;' : '';
  }
  ...
  function _ref5$2() {
    ...
    this.body.selectAll('tr').data(this.maptable.data).attr('class', function (row) {
      ...
      var tds = '';
      Object.keys(_this2.maptable.columnDetails).forEach(function (columnKey) {
  ...
  function _ref7$2(columnKey, columnMode) {
  ...
  var Table = function () {
    function Table(maptable, options) {
      ...
      this.header.selectAll('tr').data([1]).enter().append('tr').selectAll('th').data(Object.keys(this.maptable.columnDetails).map(function (k) {
        return Object.assign({ key: k }, _this.maptable.columnDetails[k]);
      })).enter().append('th').attr('class', _ref$5).attr('style', _ref2$4).text(_ref3$3).attr('id', _ref4$3).on('click', function (d) {
        if (d.sorting) {
